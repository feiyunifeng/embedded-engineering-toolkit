...
describe('上线前边界审计', () => {
  it('rejects runtime UART enum and overflow inputs', () => {
    expect(() => calculateUart({ baudRate: 9600, frameCount: 1, dataBits: 8, parity: 'bad' as any, stopBits: 1 })).toThrow();
    expect(() => calculateUart({ baudRate: 1, frameCount: Number.MAX_VALUE, dataBits: 9, parity: 'odd', stopBits: 2 })).toThrow();
  });
